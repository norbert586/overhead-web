...
// Days on which the user caught at least one aircraft. A flight row carries
// only its first and last catch timestamps, so days in between (for an
// aircraft caught repeatedly across a long span) aren't reconstructable —
// in practice every catch day also produces first/last stamps on other rows,
// so the union of both dates tracks real catch days closely.
function getCatchDays(userId: number): Set<string> {
  const rows = all<{ d: string }>(
    `SELECT DISTINCT substr(first_seen, 1, 10) as d FROM flights WHERE user_id = ?
     UNION
     SELECT DISTINCT substr(last_seen, 1, 10) FROM flights WHERE user_id = ?`,
    [userId, userId],
  );
  return new Set(rows.map((r) => r.d));
}

// Consecutive catch days ending today (or yesterday — an unbroken streak
// shouldn't read as 0 before you've been outside yet today).
function computeCatchStreak(days: Set<string>): number {
  const DAY_MS = 24 * 60 * 60 * 1000;
  let cursor = Date.now();
  const dateStr = (ms: number) => new Date(ms).toISOString().slice(0, 10);
  if (!days.has(dateStr(cursor))) cursor -= DAY_MS;
  let streak = 0;
...
export function getStatsSummary(userId: number) {
  ...
  // One row per aircraft per user: COUNT(*) is the collection size and
  // SUM(times_seen) is the total number of catches.
  const summaryRow = get<{
    total_catches: number | null; unique_aircraft: number;
    operators: number; countries: number; avg_altitude_ft: number | null;
  }>(`SELECT SUM(times_seen) as total_catches, COUNT(DISTINCT hex) as unique_aircraft,
  ...
  const row24h = get<{
    catches: number; aircraft: number; operators: number; gov_count: number;
  }>(`SELECT COUNT(*) as catches, COUNT(DISTINCT hex) as aircraft,
  ...
  const classRows = all<{
    classification: string; total_count: number | null; unique_aircraft: number;
    avg_altitude: number | null; count_24h: number;
  }>(`SELECT classification, SUM(times_seen) as total_count, COUNT(DISTINCT hex) as unique_aircraft,
  ...
  const catchDays = getCatchDays(userId);

  return {
    summary: {
      totalCatches:   summaryRow?.total_catches   ?? 0,
      ...
      streakDays:     computeCatchStreak(catchDays),
      catchDays:      catchDays.size,
    },
    summary24h: {
      catches:   row24h?.catches   ?? 0,
...
export function getStatsAircraftTypes(userId: number) {
  const typeRows = all<{
    ...
    event_count: number; unique_aircraft: number;
      ...
      SUM(times_seen) as event_count, COUNT(DISTINCT hex) as unique_aircraft
...
export function getStatsOperators(userId: number) {
  ...
  }>(`SELECT operator, SUM(times_seen) as event_count, COUNT(DISTINCT hex) as unique_aircraft,
...
export function getSessionStats(userId: number) {
  const totalRow = get<{ count: number | null }>('SELECT SUM(times_seen) as count FROM flights WHERE user_id = ?', [userId]);
  ...
  const topRows = all<{ aircraft_type: string; count: number }>(
    `SELECT aircraft_type, SUM(times_seen) as count FROM flights
